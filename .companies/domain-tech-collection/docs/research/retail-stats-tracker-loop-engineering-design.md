...
**実装設計との関係**: 本書 v0.1 は `retail-stats-tracker-design.md` の完成前に書かれ、要件定義 v0.1 を根拠にしていた。v0.1.1 で実装設計と要件定義 v0.1.1 に追随済みである。具体的には natural key の 5 要素化（§2.3 ④）、NFR-05 分母の再定義（§1.2 / §2.3 ⑦）、コード配置パスの確定（§2.1、実装設計 §2.1）を反映した。
...
| C4 | 業態表に必須列（`segment_id`/`業態ID`, `名称`/`正式名称`, `別名`/`表記ゆれ`, `種別`, `発表主体`, `表示順`）が**許容名のいずれかで**揃う | `required_column_missing` |
...
| C11 | 各業態の `発表主体` セルが IF-02 の**発表主体対応表**（先頭トークン → kebab-case コード）で写像できる。対応表にない値はエラー停止 | `authority_unmapped` |
| C12 | **C1〜C11 がすべて通ったときに限り、カタログローダ自身（実装設計 §3.3 の `Catalog.validate()`）を実行し、残る `CatalogError` を報告する** | `loader_rejected` |

C7 が「セル内トークンごと」なのは、現行カタログの `sales-amount-absolute` の単位セルが実際に `億円 / 兆円` の併記だからである（カタログ §2.1）。セル全体の完全一致で照合すると初日から fail する。

**C4 に `発表主体` を含める理由**: 要件 v0.1.1 が `発表主体` を業態定義表の必須列に昇格させた（IF-02 / 差分表 #3）。この列は natural key の第 5 要素 `source_authority` を供給する。欠けたまま契約検査を通すと、要件 7-14 とカタログ §1.4 が「重複でも取りこぼしでもなく、母集団の異なる別系列」と定義したケース（`home-center` の 業界紙集計 / 経産省、`drugstore` の 経産省 / 個社開示）が**同一キーに衝突して静かに上書きされる**。値が入れ替わるだけで例外にならないため、§1.2 の silent accumulation と同型の事故になる。現行カタログ 13 業態はすべて `発表主体` を持つため、この検査の追加によるカタログ側の改訂は発生しない。

**C12 を置く理由（ゲートとローダの包含関係）**: C1〜C11 はローダのバリデーション（実装設計 §3.3 の V1〜V13）の**真部分集合**であり、C 番号に対応のない検査が残る（V9 上位業態の参照先実在 / V10 参照の循環なし / V11 別名が空でない / V12 別名の重複なし など）。段階 0 の完了条件は「実装前にカタログ側の不整合を出し切る」ことなので、ゲートがローダより緩いと「**hook は緑なのに実行すると止まる**」状態を段階 0 が見逃す。

C 番号を V 番号に 1:1 で増やす案は採らない。ローダ側に V14 以降が追加されるたび同じ乖離が再発し、追随漏れが構造的に起きるためである（本節の C4 欠落がまさにその形で発生した）。代わりに**ローダ自身を検査の最後に通す**ことで、包含関係をコードの重複なしに保証する。

C1〜C11 を先に評価するのは、ローダは最初の `CatalogError` で停止するため、**列欠落のような自明な不備がある状態で実行すると理由コードが 1 件しか得られない**からである。C1〜C11 は独立に全件評価できるので、編集者に一度で全ての不備を返せる。
