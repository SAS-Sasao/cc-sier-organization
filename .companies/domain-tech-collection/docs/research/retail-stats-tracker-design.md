...
本書は [要件定義書 v0.1.2](retail-stats-tracker-requirements.md)（以下「要件」）が定義した「何を作るか」に対し、「どう作るか」を実装者が手を動かせる粒度まで落としたものである。要件を上位文書とし、本書は要件を変更しない。

| 項目 | 内容 |
|------|------|
| 上位文書 | `.companies/domain-tech-collection/docs/research/retail-stats-tracker-requirements.md`（**v0.1.2**） |
...
| 文書 | 役割 |
|---|---|
| [小売月次統計トラッカー 要件定義書 v0.1.2](retail-stats-tracker-requirements.md) | 上位文書。FR / NFR / データ定義 / IF-02 スキーマ契約。変更点は同書 7.1（v0.1 → v0.1.1）および 7.2（v0.1.1 → v0.1.2）を参照 |
