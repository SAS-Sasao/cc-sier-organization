...
└── analyses/              ← 個別の解析レポート
    ...
    ├── loop-engineering-proof-or-stop.md
    │                               ループエンジニアリングと Proof-or-Stop
...
**解析済みは 2026-08 の 15 日分のみです。**

| 項目 | 値 |
|---|---|
| 蓄積されているダイジェスト | **129 日分**（2026-03-21 〜 2026-08-22） |
| うち解析済み | **15 日分（11.6%）**（2026-08-01 〜 08-15） |
| 未解析 | **114 日分** |
| A1+A2 の総記事数 | **3447 件**（実測） |

> 2026-03-21 〜 2026-07-31 の約 107 日分は**読んでいません**。ここに本組織へ効く知見が残っている可能性はありますが、**サンプリングもしていないため量は不明**です。

> **「解析済み」は原文精読の意味ではありません。** 08-03〜08-09 はユニーク 181 件中 **12 件（6.6%）**、08-10〜08-15 は 168 件中 **8 件（4.8%）**しか原文を読んでいません。残りはタイトルと 1 行要約で判断しています（[`analyses/2026-08-w2-claude-code-codex-agent.md`](../../../../docs/insights/analyses/2026-08-w2-claude-code-codex-agent.md) §7 / §8.7）。
...
| **AE** | Skill 台帳の棚卸し | 🔶 一部採用 | `plugins/` に無い Skill 3 種は**未移送**。CLAUDE.md の表は 2026-08-16 に 18 種へ是正済み |
| **AO** | 候補 Q を「長さの削減」→「整合性の是正」へ | ⬜ | **1,650 回の実測で CLAUDE.md の長さは想起に効かないと判明**。本組織は 208 行 + rules 1,010 行で実験条件とほぼ一致。削る作業をやめ AD・O へ振り替える |
| **AP** | `deny` の具体設計を導入 | ⬜ | `node -e` / `sh -c` / `git -c` という**迂回経路**と、`Write(path)` ルールが参照されない仕様は自力では気づけない |
| **BC** | 停止条件を「主張」から「証拠」へ（Proof-or-Stop） | ⬜ | **`completed` 237 件中 92.4% が未検証事項を書いていない**。「AI の『終わりました』は主張であって事実ではない」。B/C/S/AL を 1 本に束ねる |
