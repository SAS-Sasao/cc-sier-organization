...
### [2026-06-13] スキル堅牢化を適用（オーナー承認 A）
v2 SKILL.md §5.3 に貫通回避の必須手順「エッジは icon-to-icon 接続」「service group は aws-cloud 配下に reparent（相対座標）」を追記（review-drawio.js の体系的 false positive 対策。初回 L0② 通過のため）。§5.1 に validate_drawio.py の "N unique shapes" 表示の癖を注記。generate-iac-viewer.py のフッターを「Powered by AWS IaC MCP Server」→「Generated by /company-diagram-v2」に修正。plugins↔.claude 同期一致(diff -rq クリーン)。ドライ実行4成果物(docs/diagrams/dryrun-v2-ec-serverless.*)を証拠として PR #573 にコミット。
