...
| 機能 | コマンド / Skill | 説明 |
|---|---|---|
| ① 組織を作る・入る | `/company {org-slug}` | 新規組織を作成、または既存組織に切り替え |
...
`/company {org-slug}` を実行するだけで組織を作れます。存在しない org-slug を指定すると新規組織を自動構築し、存在する場合はそこに切り替えます。
