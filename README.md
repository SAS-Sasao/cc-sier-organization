...
**⑬ WBS を SSoT にした三層 TODO 管理**
WBS markdown（master）→ GitHub Issues（派生）→ GitHub Projects v2（可視化）の3層構造で、全組織のWBSから毎朝自動同期されます。閉じた Issue は WBS の `[x]` に自動反映され、翌日のTODO候補から自動除外されます。

**⑭ 対話型 TODO dispatcher で朝イチの意思決定を楽に**
`/company-today` を叩くと、今日の Kanban から **組織 → タスク → 対応方針** の3段階選択で、学習支援・ハンズオン・subagent委譲・壁打ちのどれに進むかを対話的に決められます。

**⑮ 完了件数と WBS 進捗をブラウザで可視化**
`TodoInsights` ダッシュボード（Chart.js）で日別完了件数・週次トレンド・カテゴリ別・組織別・累計完了、そして **WBS全体進捗バー**（完了/進行中/未着手）を一覧できます。毎晩 23:50 JST に自動更新されます。

**⑯ GitHub Actions 6 workflow による完全自動運転**
朝夜の cron で TODO同期・Kanban補充・日次ダイジェスト生成・CLAUDE.md学習更新・TodoInsights収集が自動実行されます。ローカル `/company-cycle` と組み合わせて、手を動かす時間を最大化できます。
...
| ㉔ 対話型 TODO dispatcher | `/company-today` | Project v2 のオープンTODOから「組織→タスク→対応方針」の3段階選択で対応開始 |
| ㉕ 日次ダイジェスト統合実行 | `/company-daily-digest` | 巡回→MD→L1/L2レビュー→PR auto-merge→HTML公開を1コマンド完結（8 Phase） |
| ㉖ cycle オーケストレータ | `/company-cycle` | `/company-report` → `/company-evolve` → `/company-dashboard` を直列実行 |
| ㉗ WBS → Issues 同期 | `daily-todo-sync.yml` (Actions) | 毎朝 05:00 JST、全組織 WBS markdown を GitHub Issues に同期 + AI による Top N 選定 |
| ㉘ 今日の kanban 自動補充 | `daily-kanban-sync.yml` (Actions) | 毎朝 05:30 JST、Project v2 TODOKANBAN に今日の top 5 WBS タスクを組織バランスで反映 |
| ㉙ TodoInsights 自動収集 | `daily-insights-sync.yml` (Actions) | 毎晩 23:50 JST、closed Issues/PRs を収集し TodoInsights Project + HTML ダッシュボードを更新 |
| ㉚ TodoInsights ダッシュボード | Chart.js | 日別・週次・カテゴリ・組織別・累計・WBS全体進捗を可視化しGitHub Pagesで公開 |
...
│   │   ├── company-daily-digest/   # /company-daily-digest Skill（巡回〜HTML公開8 Phase統合）
│   │   ├── company-cycle/          # /company-cycle Skill（report→evolve→dashboardオーケストレータ）
│   │   ├── company-today/          # /company-today Skill（対話型TODO dispatcher）
│   │   └── company-handover/       # /company-handover Skill（ナレッジポータル生成）
│   ├── rules/                       # 運用ルール（CLAUDE.mdから @参照）
│   │   ├── git-workflow.md          # ブランチ・コミット・PR・auto-merge
│   │   ├── task-log.md              # タスクログYAML形式・Issue自動作成
│   │   ├── multi-org.md             # マルチ組織運用
│   │   ├── artifact-placement.md    # 成果物配置マトリクス
│   │   ├── review-pattern.md        # L0/L1/L2 3層レビュー設計
│   │   ├── skill-development.md     # SKILL.md制約・plugins↔.claude同期
│   │   ├── todo-management.md       # TODO管理ルール全組織共通
│   │   └── wbs-schema.md            # WBS markdown拡張スキーマ定義
│   └── hooks/
...
│       ├── sync-board.sh            # タスクログ/WBSからboard.md同期
...
│       ├── parse-wbs.py              # WBS markdown header-aware parser
│       ├── migrate-wbs-schema.py     # WBS スキーマ拡張ツール（Iter/Pri/Type/Issue列追加）
│       ├── bootstrap-todo-issues.sh  # WBS→Issue バルク作成
│       ├── daily-kanban-sync.py      # Project v2 TODOKANBAN 同期
│       ├── daily-insights-sync.py    # GitHub Search→TodoInsights Project同期
│       └── generate-insights-html.py # TodoInsights HTMLダッシュボード生成（Chart.js）
│
├── .github/
│   └── workflows/                   # GitHub Actions 自動化（6本のcron workflow）
│       ├── daily-todo-sync.yml      # 05:00 JST WBS→Issues同期
│       ├── daily-kanban-sync.yml    # 05:30 JST TODOKANBAN補充
│       ├── daily-digest-automation.yml # 07:30 JST 日次ダイジェスト自動化
│       ├── daily-cycle-supplement.yml  # 19:00 JST /company-cycle リマインダー
│       ├── nightly-claude-md-update.yml # 23:00 JST CLAUDE.md/rules 学習更新
│       └── daily-insights-sync.yml  # 23:50 JST TodoInsights収集・HTML再生成
│
...
│   ├── drawio/                    # draw.ioダイアグラムギャラリー（GitHub Pages）
│   │   ├── index.html             # 一覧ページ
│   │   └── {name}.drawio / .html # 各図のXMLとビューアHTML
│   ├── daily-digest/              # 日次ダイジェストHTML（GitHub Pages）
│   │   └── index.html             # タブ切替SPAページ
│   ├── insights/                  # TodoInsightsダッシュボード（GitHub Pages）
│   │   └── index.html             # Chart.jsによる完了件数・WBS進捗の可視化
...
| TodoInsights | [/insights/](https://sas-sasao.github.io/cc-sier-organization/insights/index.html) | 日々のクローズ件数・カテゴリ別・組織別・累計・WBS全体進捗をChart.jsで可視化。毎晩23:50 JSTに自動更新 |

ダッシュボードは `/company-dashboard` で生成、日次ダイジェストは `/company-digest-html` で生成、構成図は `/company-diagram` と `/company-drawio` で生成、ナレッジポータルは `/company-handover` で生成されます。TodoInsightsは `daily-insights-sync.yml` ワークフローが自動生成します。

---

## GitHub Actions 自動運転

`.github/workflows/` に 6 本の cron workflow が配置されており、毎日自動発火して TODO 管理・ダイジェスト生成・インサイト収集・学習更新を完全自動化します。

| 時刻 (JST) | Workflow | 役割 |
|---|---|---|
| 05:00 | `daily-todo-sync.yml` | 全組織 WBS markdown → GitHub Issues 同期 + AI による Top N 選定 + board.md 再生成 |
| 05:30 | `daily-kanban-sync.yml` | Project v2 `TODOKANBAN` に今日の top 5 WBS タスクを反映（組織バランス + 未完了タスクの繰越し） |
| 07:30 | `daily-digest-automation.yml` | `/company-daily-digest` 8 Phase 統合実行（巡回 → MD → L1/L2レビュー → PR auto-merge → HTML公開） |
| 19:00 | `daily-cycle-supplement.yml` | git tracked データの軽量サマリー生成 + `/company-cycle` 実行リマインダー |
| 23:00 | `nightly-claude-md-update.yml` | CLAUDE.md と `.claude/rules/*.md` を学習シグナルから自動更新（差分時のみ PR 作成） |
| 23:50 | `daily-insights-sync.yml` | `TodoInsights` Project に closed Issues/PRs を収集 + Chart.js ダッシュボード再生成 |

各 workflow は**継続トラッキング Issue** に実行ログを自動コメント投稿するので、朝起きた時に状態を一覧で確認できます。

認証:
- `CLAUDE_CODE_OAUTH_TOKEN` (Max plan OAuth) : claude-code-action@v1 用
- `PROJECTS_PAT` (Classic PAT, scope: `project` + `read:org` + `read:discussion`) : Project v2 操作用（user-owned Project v2 は fine-grained PAT 非対応のため）
