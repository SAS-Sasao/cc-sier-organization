...
**サービス選定**: ダイアグラムに配置するAWSサービスは `references/aws-service-defaults.md` を参照して選定する。
ユーザーが明示的にサービスを指定していない場合、アプリパターン（Webアプリ/SPA/API等）からデフォルトサービスを自動選定する。
ユーザーが Override Trigger に該当するキーワード（"serverless"、"simple RDS" 等）を使った場合はオーバーライドする。
...
レビュー Pass 後、ダイアグラムのAWSサービス構成からコスト概算を算出する。
`references/aws-cost-estimation.md` のクイックリファレンス見積を参照し、Dev / Prod の月額概算を計算する。

#### 3.4.1 算出フロー

1. ダイアグラムコードから使用AWSサービスを抽出
2. `aws-cost-estimation.md` の該当サービス単価を参照
3. Dev / Prod それぞれの月額レンジを算出
4. `awspricing` MCP が利用可能であれば、リアルタイム価格で補正（任意）
5. 詳細ページの「コスト概算」セクション（`aws-cost-estimation.md` の表示フォーマット参照）に記載

#### 3.4.2 コスト��算テーブル

詳細ページ（{filename}.html）の「設計のポイント」セクションの後に追加:

```
コスト概算 セクション:
- サービス別内訳テーブル（サービス / 構成 / Dev月額 / Prod月額）
- 合計行
- 前提条件（リージョン、稼働時間、転送量想定等）
- コスト最適化のポイント（Savings Plans、スケジュール停止、Serverless活用等）
```

### 3.5 IaCソースコード生成

レビュー Pass + コスト概算後、ダイアグラムの構成を CloudFormation YAML で実装する。
...
#### 3.5.5 コードビューアページ（{filename}-iac.html）
...
| ファイル | 用途 | 参照タイミング |
|---------|------|-------------|
| `references/aws-service-defaults.md` | AWSサービス選定基準（Compute/DB/Storage） | ダイアグラム生成時（3.2） |
| `references/aws-cost-estimation.md` | コスト見積パターン（サービス別単価・構成パターン別概算） | コスト概算時（3.4）、詳細ページ生成時 |
| `references/generate-iac-viewer.py` | IaCコードビューアHTML生成スクリプト | IaC生成時（3.5.5） |

> `aws-service-defaults.md` / `aws-cost-estimation.md` は [awslabs/agent-plugins](https://github.com/awslabs/agent-plugins) deploy-on-aws (Apache-2.0) をベースに拡張
