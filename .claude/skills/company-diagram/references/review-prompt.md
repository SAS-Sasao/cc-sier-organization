...
2. **Read ツールで詳細HTMLを読み込む** — **必須7セクション**（凡例/概要/データフロー/レイヤー構成/**設計のポイント**/**コスト概算**/**学習ポイント**）の存在と順序を確認
...
6. 学習ポイントセクションの内容妥当性を確認（3〜5 項目、設計のポイントと重複せず transferable な知見か）
...
**重要**: 必須セクションのうち**1 つでも欠落している場合**、または**順序が違う場合**、他のスコアに関わらず **`critical_triggered = true` かつ `verdict = "fail"` を強制する**こと。
