...
> 用途: ダイアグラム生成時にアプリパターンからAWSサービスをデフォルト選定する
...
| Document / MongoDB | Amazon DocumentDB Serverless | Amazon DocumentDB Serverless | "provisioned" → DocumentDB provisioned |
...
If user's repo already has `terraform/` or `cdk.json`, match existing choice.
