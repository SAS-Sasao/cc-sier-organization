...
他の組織への意図しない変更を含めないよう、組織ディレクトリを明示指定します。

> **注**: `.companies/.active` はローカル設定ファイル（.gitignore で除外）のため、`git add` しない。
...
```
design: メダリオンアーキテクチャ設計書を作成 [a-sha-dwh] by tanaka
docs: キックオフ議事録を追加 [shakai-it] by sasao
admin: アーキテクチャ室を追加 [hyojunka] by suzuki
todo: スプリント1 TODOリストを作成 [shakai-it] by tanaka
feat: 品質チェックワークフローを追加 [a-sha-dwh] by sasao
```

#### 実行コマンド

```bash
git commit -m "{type}: {概要} [{org-slug}] by {operator}"
```
...
# コンフリクト解決後
git add .companies/{org-slug}/
git commit -m "fix: コンフリクトを解決 [{org-slug}] by {operator}"
git push origin {branch-name}
```
