...
L0 が fail した場合、**生成物の誤り**と**ゲート自体のバグ（false-positive）**を先に切り分ける。成果物を修正する前にゲート側の検証を行う。

**切り分け手順**:
1. L0 スクリプトのエラーメッセージを確認し、指摘箇所を特定
2. 生成物（xlsx / HTML / drawio 等）を直接開いて指摘が正当か目視確認
3. 正当 → 生成物を修正して再実行
4. 誤判定 → ゲートスクリプトを修正し、生成物は無修正で再実行

**過去の事例**: 2026-06-14 `/company-sheet` 初回実運用で L0 が `=SUM(monthly!E2:E18)` の終端セル `E18` を「現シートの参照」と誤判定。xlsx は正しく、L0 チェッカのレンジ解析がシート修飾子の継承を考慮していなかった（PR #584）。生成物を無修正のまま L0 チェッカ側を修正して解決。
