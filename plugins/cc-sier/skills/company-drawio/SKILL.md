...
5. **エッジ貫通レビュー**: `.drawio` ファイルに対してレビュースクリプトを実行し、問題があればXMLを修正して再保存

### 4.2.1 エッジ貫通レビュー

`.drawio` ファイル保存後、以下のコマンドでエッジがノードを貫通していないかチェックする:
...
- 終了コード `1`: 貫通検出 → レポートを確認しXMLのノード配置を修正して再実行
- 修正方法: ノード間の間隔を広げる、またはエッジにwaypoint（中間点）を追加する
